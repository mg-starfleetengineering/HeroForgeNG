...
export async function importRosterPackage(parsed: any): Promise<{ importedCount: number; lastImportedId: string | null }> {
  ...
  if (parsed.exportType === 'heroforge_roster_backup' && Array.isArray(parsed.characters)) {
    characterArray = parsed.characters;
  } else if (parsed.exportType === 'heroforge_roster_backup' && parsed.characters && typeof parsed.characters === 'object') {
    characterArray = Object.values(parsed.characters);
  } else if (Array.isArray(parsed)) {
    characterArray = parsed;
  } else if (parsed.name || parsed.levelProgression || parsed.baseStats) {
    characterArray = [parsed];
  }
